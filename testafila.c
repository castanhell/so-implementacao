//------------------------------------------------------------------------------
// Programa de teste da implementacao de fila generica queue.c - queue.h.
...
// imprime na tela um elemento da fila (chamada pela funcao queue_print)
...
// retorna 1 se a estrutura da fila está correta, 0 senao
int fila_correta (filaint_t *fila)
{
   filaint_t *aux ;

   // uma fila vazia sempre esta correta
   if (!fila)
      return 1 ;

   // fila com um so elemento e correta
   if ((fila->next == fila) && (fila->prev == fila))
      return 1 ;

   // fila com um so elemento, mas incorreta
   if ((fila->next == fila) || (fila->prev == fila))
   {
   ...
   do
   {
      // testa ponteiro next (avaliacao em curto-circuito)
      ...
      // testa ponteiro prev (avaliacao em curto-circuito)
...
int main (int argc, char **argv, char **envp)
{
   ...
   do
   {
      assert (i == aux->id) ; // testa posicao do elemento i
      i++ ;
      aux = aux->next ;
   } while (aux != fila0) ;
   ...
   // esvazia fila0, retirando sempre o ultimo elemento
   ...
   // tentar remover elemento que esta em outra fila
   ...
   // tentar remover elemento que nao esta em nenhuma fila
   ...
   // tentar inserir algo que ja esta na mesma fila
   ...
   // tentar inserir algo que esta em outra fila
